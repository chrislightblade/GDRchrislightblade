 ...
 */
public class ClasseGioco_old {

    //private ClasseGioco_old classe;
    String talenti[];// = new String[5];
    int talentoAttivo[];// = new int[]{0, 0, 0, 0, 0};

    public ClasseGioco_old() {
        this.talenti = new String[5];
        this.talentoAttivo = new int[]{0, 0, 0, 0, 0};
    }

    /*public ClasseGioco_old(int i, SchedaPersonaggio scheda) {
        if(i == 1)
          ClasseGioco_old.ClasseArmigero a = new ClasseGioco_old.ClasseArmigero(scheda);  
    }*/
 /*public ClasseGioco_old getClasse() {
        return classe;
    }

    public void setClasse(ClasseGioco_old classe) {
        this.classe = classe;
    }*/
    ...
    public void usaTalento(SchedaPersonaggio scheda1, ArrayList<SchedaPersonaggio> schedaMobs) {
        ...
        for (int k = 0; k < 5; k++) {// i talenti non ancora attivi e funzionanti hanno valore 0, mentre quelli attivi hanno valore 1. Stampo gli 1
            if (talentoAttivo[k] == 1 && classe.getUso(k) > 0) {
                frase += (k + 1) + "." + talenti[k] + "\n";
            }
        }
